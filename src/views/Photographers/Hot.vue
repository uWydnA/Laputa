<template>
  <div>
    ...
    <van-pagination v-model="currentPage" :page-count="25" mode="simple" @change="handleChange()" />
  </div>
</template>
<script>
import Vue from 'vue'
...
export default {
  ...
  mounted () {
    this.$axios({
      url: `/api/v2/photographers/hot?user_type=&lang=zh-Hans&platform=web&device=mobile&limit=20&offset=${this.page}`
    }).then(res => {
      // console.log(res)
